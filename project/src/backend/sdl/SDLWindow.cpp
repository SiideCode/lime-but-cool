...
namespace lime
{
	...
	SDLWindow::SDLWindow(Application *application, int width, int height, int flags, const char *title)
	{
		...
		if (flags & WINDOW_FLAG_SKIP_TASKBAR)
			sdlWindowFlags |= SDL_WINDOW_SKIP_TASKBAR;
		// these work only with X11
		if (flags & WINDOW_FLAG_POPUP_MENU)
			sdlWindowFlags |= SDL_WINDOW_POPUP_MENU;
		if (flags & WINDOW_FLAG_UTILITY)
			sdlWindowFlags |= SDL_WINDOW_UTILITY;
		if (flags & WINDOW_FLAG_TOOLTIP)
	...
	bool SDLWindow::CreateTrayIcon(const char *resourcePath)
	{
		...
		{
			tIcon.uCallbackMessage = WM_APP + SDL_GetWindowID(sdlWindow);
